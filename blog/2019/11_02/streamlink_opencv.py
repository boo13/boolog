...
def detect_faces(cascade, frame, scale_factor=1.1, min_neighbors=5):
    ...
    faces = cascade.detectMultiScale(
        frame_gray, scaleFactor=scale_factor, minNeighbors=min_neighbors
    )
...
def main(url, quality="best", fps=30.0):
    face_cascade = cv2.CascadeClassifier(
        os.path.join(cv2.haarcascades, "haarcascade_frontalface_default.xml")
    )

    stream_url = stream_to_url(url, quality)
    logger.info("Loading stream {0}".format(stream_url))
